...
SCOPE = "playlist-modify-public playlist-modify-private user-read-recently-played user-top-read user-library-modify"
STATE = ""
...
if sys.version_info[0] >= 3:
    URL_ARGS = "&".join(
        [
            "{}={}".format(key, urllibparse.quote(val))
...
def get_users_top(auth_header, t):
    if t not in ["artists", "tracks"]:
        print("invalid type")
        return None
    url = "{}/{type}?limit=2&time_range=short_term".format(USER_TOP_ARTISTS_AND_TRACKS_ENDPOINT, type=t)
    resp = requests.get(url, headers=auth_header)
    return resp.json()
...
def put_new_saved_track(auth_header, track_id):
    url = "{}/tracks".format(USER_PROFILE_ENDPOINT)
    data = {"ids": [track_id]}

    resp = requests.put(url, data=json.dumps(data), headers=auth_header)
...
def get_recommended_tracks(auth_header, artists, tracks, genre):
    url = "{}/?seed_artists={seed_artists}&seed_tracks={seed_tracks}&seed_genres={seed_genre}".format(
